...
class SafetyAlertPostScreen extends StatefulWidget {
  final PostDetailsControllerCubit? postDetailsControllerCubit;
  const SafetyAlertPostScreen({super.key, this.postDetailsControllerCubit});

  static const routeName = 'safety_alert_post';

  @override
  State<SafetyAlertPostScreen> createState() => _SafetyAlertPostScreenState();
}

class _SafetyAlertPostScreenState extends State<SafetyAlertPostScreen> {
  bool get isEdit => widget.postDetailsControllerCubit != null;
  SafetyPostModel? existingSafetyAlertPostDetails;
...
//
  ...
  void updatePostController() {
    existingSafetyAlertPostDetails!.title = titleTextController.text.trim();
    existingSafetyAlertPostDetails!.description =
        descriptionTextController.text.trim();
    existingSafetyAlertPostDetails!.media = serverMedia;
    existingSafetyAlertPostDetails!.taggedlocation = taggedLocation;
  ...
  void onPost() {
    if (taggedLocation == null) {
      ...
      final safetyDataPostModel = SafetyDataPostModel(
        id: existingSafetyAlertPostDetails?.id,
        postType: PostType.safety,
      ...
      uploadRegularPostCubit.manageSafetyPost(
  ...
  void prefillData({required FeedRadiusModel feedRadiusModel}) {
    existingSafetyAlertPostDetails = widget
        .postDetailsControllerCubit!.state.socialPostModel as SafetyPostModel;

    titleTextController.text = existingSafetyAlertPostDetails!.title;

    descriptionTextController.text =
        existingSafetyAlertPostDetails!.description;

    //Post settings
    postSharePermission = existingSafetyAlertPostDetails!.postSharePermission;
    postVisibilityPermission =
        existingSafetyAlertPostDetails!.postVisibilityPermission;
    postCommentPermission =
        existingSafetyAlertPostDetails!.postCommentPermission;

    //Tag location
    setTaggedLocation(
      selectedTaggedLocation: existingSafetyAlertPostDetails!.taggedlocation,
    );

    //server media
    serverMedia = existingSafetyAlertPostDetails!.media;

    //emitting radius data
    radiusSliderCubit.emitRadius(feedRadiusModel.copyWith(
      socialMediaSearchRadius:
          existingSafetyAlertPostDetails!.postPreferenceRadius,
    ));
    postButtonValidator();
  }
  ...
  void postButtonValidator() {
    if (isEdit &&
        ...
        existingSafetyAlertPostDetails!.media.isEmpty) {
                ...
                body: ListView(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child:
                          ScreenHeader(title: tr(LocaleKeys.safetyAndAlerts)),
                    ),
                    ...
                    CaptionBoxWidget(
                      ...
                      hintText: tr(LocaleKeys.describeIncident),
                      onChanged: () {
                        postButtonValidator();
                      },
                      onTap: () {
